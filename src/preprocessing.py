...
def create_graph(nodes, ratings):
    ...
    train_g.edges['is_rating'].data['r'] = torch.tensor(
        train_ratings).reshape(-1, 1)
    val_g.edges['is_rating'].data['r'] = torch.tensor(
        val_ratings).reshape(-1, 1)
    test_g.edges['is_rating'].data['r'] = torch.tensor(
        test_ratings).reshape(-1, 1)

    train_g.edges['is_rated'].data['r'] = torch.tensor(
        train_ratings).reshape(-1, 1)
    val_g.edges['is_rated'].data['r'] = torch.tensor(
        val_ratings).reshape(-1, 1)
    test_g.edges['is_rated'].data['r'] = torch.tensor(
        test_ratings).reshape(-1, 1)

    return train_g, val_g, test_g


def split_dataset_movielens(data_train, data_test, dtypes):
    ...
    train_ratings = np.subtract(
        data_array_train[:, 2].astype(dtypes['ratings']), 1)

    test_u_nodes = data_array_test[:, 0].astype(dtypes['u_nodes'])
    test_v_nodes = data_array_test[:, 1].astype(dtypes['v_nodes'])
    test_ratings = np.subtract(
        data_array_test[:, 2].astype(dtypes['ratings']), 1)
    ...
    train_ratings = np.subtract(
        data_array_train[train_idx, 2].astype(dtypes['ratings']), 1)

    val_u_nodes = data_array_train[val_idx, 0].astype(dtypes['u_nodes'])
    val_v_nodes = data_array_train[val_idx, 1].astype(dtypes['v_nodes'])
    val_ratings = np.subtract(
        data_array_train[val_idx, 2].astype(dtypes['ratings']), 1)

    train_nodes = np.hstack(
        [train_u_nodes.reshape(-1, 1), train_v_nodes.reshape(-1, 1)])  # 학습 타겟 대상
    test_nodes = np.hstack(
        [test_u_nodes.reshape(-1, 1), test_v_nodes.reshape(-1, 1)])  # 학습 타겟 대상

    nodes = (train_u_nodes, train_v_nodes, val_u_nodes,
             val_v_nodes, test_u_nodes, test_v_nodes)
...
# def split_dataset_default(data_train, data_test, dtypes):
#     data_array_train = data_train.values.tolist()
#     data_array_train = np.array(data_array_train)
#     data_array_test = data_test.values.tolist()
#     data_array_test = np.array(data_array_test)
...
def load_ml_100k(data_path):
    sep = '\t'
    dtypes = {'u_nodes': np.int32, 'v_nodes': np.int32,
              'ratings': np.int64, 'timestamp': np.float64}
    ...
    train_g, val_g, test_g, train_nodes, test_nodes, train_ratings, test_ratings = split_dataset_movielens(
        data_train, data_test, dtypes)
